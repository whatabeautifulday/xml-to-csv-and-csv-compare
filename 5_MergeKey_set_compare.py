...
set_diff_df1 = pd.concat([b, sort_df1, sort_df1]).drop_duplicates(keep=False) #df2 - df1
set_diff_df2 = pd.concat([b, sort_df2, sort_df2]).drop_duplicates(keep=False) #df1 - df2
set_diff_df1.to_csv(f'{today}_old_set_diff.csv')
set_diff_df2.to_csv(f'{today}_new_set_diff.csv')
